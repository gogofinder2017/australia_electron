...
function substitute(content, urls, replacements) {
	...
	urls.forEach(function (url, i) {
		if (url && replacements[i]) {
			content = content.replace(decodeURI(url), replacements[i]); //Added to catch incorrect urls.
			content = content.replace(new RegExp(url, "g"), replacements[i]);
		}
	});
	return content;
...
var Contents = function () {
	function Contents(doc, content, cfiBase, sectionIndex, isVerticalWriting/*RDD*/) {
		...
		this.window = this.document.defaultView;
                ...
                if(isVerticalWriting === 'none') {
                    let verticalMode = check_writingMode($(this.documentElement));
                    isVerticalWriting = verticalMode;
                    window.reader.setVertical(verticalMode);
                    window.reader.book.settings.isVerticalWriting = verticalMode;
                    window.reader.rendition.settings.isVerticalWriting = verticalMode;
                }
                if(!isVerticalWriting)// amend by jane
		...
		//RDD
		this.isVerticalWriting = isVerticalWriting;
	...
	_createClass(Contents, [{
  	...
  	*/
		...
		/* because some books set css in seperate nodes
		** so if only set css in body style
		** its priority will lower than seperate node style
		** so this function will set css into every nodes
		** forced to adjust font-size, font-family or line-height */
		key: "cssEveryNodes",
		value: function cssEveryNodes(property, value) {
			var content = this.content || this.document.body;
			...
			while(len != 0){
				...
				for(i=0; i<len; i++){
					...
					if(nextLevelLen == 0){
						this.setNodeCss(childList[i], property, value);
					}else{
		...
		key: "setNodeCss",
		value: function setNodeCss(node, property, value) {
			var isPercentage = (value.slice(-1))=="%" ? true : false;
			if (node.style) {
				if(isPercentage){
					...
					if(property == "line-height" && originValue == "normal"){
						originValue = this.getLineHeight(node);
					}
					...
					var newValue;
					if(percentage > 100){
						newValue = String(num+2) + unit;
						// newValue = String(Math.ceil(num*percentage/100)) + unit;
					}else{
						newValue = String(num-2) + unit;
						// newValue = String(Math.floor(num*percentage/100)) + unit;
					}
		...
		key: "getLineHeight",
		value: function getLineHeight(element) {
		...
		key: "css",
			value: function css(property, value, priority) {
                        // Jane 原先使用函式統一放至finder.js整理    
                        set_css(this, property, value, priority);
                            return;
                            
                        // Jane 下列不使用
			var content = this.content || this.document.body;
                       
				if (value && property != "transform" && property != "transform-origin" && property != "line-height") {
			...
			//RDD N
				if (property == "transform-origin"){
					...
					items.forEach(function (element) {
						...
						if (elementStyle.backgroundSize) {
							elementStyle.transformOrigin = value;	
							backgroundExisted = true;
						}
					});
					if (!backgroutransformndExisted) {
						content.style.setProperty(property, value, priority ? "important" : "");
					}
				...
				if(value && (property == "font-size"  || property == "line-height" || property == "font-family" || property == "color")) {
					this.cssEveryNodes(property, value);
				...
				//** by RDD
                    ...
                    //RDD
                    var i,j;
                    if(value && (property == "font-size"  || property == "line-height" || property == "font-family")){
                        var isPercentage = (value.slice(-1))=="%" ? true : false;
                        var percentage;
                        if(isPercentage){
                            percentage= Number(value.match(/\d+/g)[0]);
                        }
                            ...
                            for(i=0; i<len; i++){
                                ...
                                if(nextLevelLen == 0){
                                    if (childList[i].style) {
                                        ...
                                        if(isPercentage){
                                            var originValue = window.getComputedStyle(childList[i]).getPropertyValue(property);
                                            var num, unit;
                                            if(property == "line-height" && originValue == "normal"){
                                                originValue = this.getLineHeight(childList[i]);
                                                console.log(originValue);
                                                // num = lineHeight;
                                                // unit = "";
                                            }/*else{
                                                num = Number(originValue.match(/\d+/g)[0]);
                                                unit =  originValue.match(/[a-zA-Z]+/g)[0];
                                            }*-/
                                            var num = Number(originValue.match(/\d+/g)[0]);
                                            var unit =  originValue.match(/[a-zA-Z]+/g)[0];
                                            ...
                                            if(percentage > 100){
                                                value = String(Math.ceil(num*percentage/100)) + unit;
                                            }else{
                                                value = String(Math.floor(num*percentage/100)) + unit;
                                            }

                                        }
                                        console.log("scale: " + value);
                                        console.log("b:" + window.getComputedStyle(childList[i]).getPropertyValue(property));
                                        childList[i].style.setProperty(property, value, priority ? "important" : "");
                                        console.log("a:" + window.getComputedStyle(childList[i]).getPropertyValue(property));
                                    }
                                }else{
                                    for(j=0; j<nextLevelChildren.length; j++ ){
                                        nextLevelChildList.push(nextLevelChildren[j]);
   ...
   */
		...
		value: function addStylesheetRules(rules) {
			...
			// Check if link already exists 
			styleEl = this.document.getElementById(/*"#" + */key); // amend by jane
			...
			if (!styleEl) {
				...
				styleEl.id = key;
                                ...
                                this.document.head.appendChild(styleEl); // amend by jane
			}
                        /* amend by jane --> 避免重覆增加 
			// Append style element to head
   ...
   */
		...
		value: function columns(width, height, columnWidth, gap) {
			...
			var axis = writingMode.indexOf("vertical") > -1/* === 0*/ ? "vertical" : "horizontal";  // amend by jane
			...
			//this.css(COLUMN_AXIS, axis/*"horizontal"*/); // amend by jane
   ...
   * @param {number} IsheightScale //RDD N
   */

	}, {
		key: "scaler",
			
			value: function scaler(scale, offsetX, offsetY, IsheightScale) {
			...
			if(IsheightScale)
			{
                                $(this.documentElement.children[1]).css("transform-origin", "top");
				//this.css("transform-origin", "top");
				offsetX = 0;
			}
			else{
                                $(this.documentElement.children[1]).css("transform-origin", "top left");
				//this.css("transform-origin", "top left");	
			}
			...
			//RDD N
                        $(this.documentElement.children[1]).css("transform", translateStr + scaleStr)
			//this.css("transform",translateStr + scaleStr);	
			//this.css("transform", scaleStr + translateStr);
                            */
   ...
   */
		...
		value: function fit(width, height) {
			...
			var offsetY = (height - viewport.height * scale) / 2 / scale;
			var offsetX = (width  - viewport.width * scale ) /2;//RDD N
			this.layoutStyle("paginated");
			...
			//RDD N
			var elements = this.content.getElementsByTagName("div");
			var items = Array.prototype.slice.call(elements);
			...
			items.every(function (element) {
				var elementStyle = element.style;
				if (elementStyle.backgroundSize) {
					IsheightScale = widthScale < heightScale ? false : true;			
				}
			});
			//RDD N
			//this.scaler(scale, 0, offsetY);
			this.scaler(scale, offsetX, offsetY, IsheightScale);
...
*/
			...
			var heightScale = height / parseInt(viewport.height);
			var scale = widthScale < heightScale ? widthScale : heightScale;
                        var offsetX = 0;//(width - viewport.width * scale) / 2 / scale;     // amend by jane
			var offsetY = 0;//(height - viewport.height * scale) / 2 / scale;   // amend by jane
			this.layoutStyle("paginated");
			this.width(parseInt(viewport.width)/*width*/);      // amend by jane
			this.height(parseInt(viewport.height)/*height*/);   // amend by jane
			this.overflow("hidden");
                        // amend by jane -->
			...
			this.scaler(scale, offsetX, offsetY);
                        ...
                        get_contentSize(viewport, scale);   // amend by jane
		}
   ...
   */
		...
		value: function writingMode(/*mode*/) {
			...
			*/
                        ...
                        // amend by jane -->
                        if(this.isVerticalWriting === true && this.documentElement) {
				this.documentElement.style[WRITING_MODE] = 
                                        (broswer.broswer === 'Safari') ? "vertical-lr" :    // 此處若不這樣設定safari 中文字會向右轉90度
                                        (originDir === "rtl") ? "vertical-rl": "vertical-lr";
                                this.documentElement.style["-webkit-"+WRITING_MODE] = 
                                        (broswer.broswer === 'Safari') ? "vertical-lr" :    // 此處若不這樣設定safari 中文字會向右轉90度
                                        (originDir === "rtl") ? "vertical-rl": "vertical-lr";
                                this.documentElement.getElementsByTagName("body")[0].style["writing-mode"] = 
//                                        (broswer.broswer === 'Safari') ? ""://"vertical-lr" :
                                        (originDir === "rtl") ? "vertical-rl": "vertical-lr";
                                this.documentElement.getElementsByTagName("body")[0].style["-webkit-writing-mode"] = 
//                                        (broswer.broswer === 'Safari') ? ""://"vertical-lr" :
                                        (originDir === "rtl") ? "vertical-rl": "vertical-lr";
                                    
			} else if(this.isVerticalWriting === false && this.documentElement) {
                                this.documentElement.style[WRITING_MODE] = "horizontal-tb";
                                this.documentElement.style["-webkit-"+WRITING_MODE] = "horizontal-tb";
                                this.documentElement.getElementsByTagName("body")[0].style["writing-mode"] = 'horizontal-tb';
                                this.documentElement.getElementsByTagName("body")[0].style["-webkit-writing-mode"] = 'horizontal-tb';
                        }
	...
	_createClass(DefaultViewManager, [{
		...
		value: function render(element, size) {
			...
			this.settings.size = size;
                        this.settings.axis = (this.settings.isVerticalWriting) ? "vertical":"horizontal";//amend by jane
			// Save the stage
   ...
   */
		...
		value: function display(section, target) {
			...
			var displayed = displaying.promise;
                        section = turn_double(section) || section;  // amend by jane
			...
			this.add(section).then(function (view) {
				...
				if (this.layout.name === "pre-paginated" && this.layout.divisor > 1) {
                                        next = section.next();
					if (next && display_nextPage(section)) {    //amend by jane
						return this.add(next);
					} else {
                                            display_singlePage(section);
                                        }
				}
		...
		value: function next() {
			...
			if (this.isPaginated && this.settings.axis === "horizontal" && (!dir || dir === "ltr")) {
				this.scrollLeft = this.container.scrollLeft;
                            ...
                            */
                                  ...
                                  // amend by jane -->
                                    left = Math.ceil(this.container.scrollLeft + this.layout.delta);
                                    left *= ($(this.container).children(".epub-view").length);
                                ...
                                // <-- amend by jane 
                                    ...
                                    if (left > 0 /*amend by jane -->*/&& (broswer.broswer !== 'Safari' || broswer.broswer !== 'Firefox')) {
                                            this.scrollBy(this.layout.delta, 0, true); 
                                    // amend by jane -->
                                    } else if(left > -this.container.scrollWidth + this.layout.delta + 1 && this.container.scrollWidth / divider !== this.layout.delta && broswer.broswer === 'Safari'){
                                            this.scrollBy(this.layout.delta/* + this.layout.gap*/, 0, true); 
                                    } else if(left > Math.ceil(-this.container.scrollWidth + this.layout.delta + 1) && (this.container.scrollWidth / divider != Math.ceil(this.layout.delta)) && broswer.broswer === 'Firefox'){
                                            this.scrollBy(this.layout.delta, 0, true); 
                                    // <-- amend by jane
                                    } else {
                                            next = this.views.last().section.next();
                                ...
                                // <-- amend by jane 
                                    ...
                                    if (top < this.container.scrollHeight) {
                                        //amend by jane -->
                                            ...
                                            let height = Math.ceil(this.layout.height - padding.top - padding.bottom + this.layout.gap);
                                            this.scrollBy(0, height, true);
                                        // <-- amend by jane
			...
			if (next) {
				...
				return this.append(next).then(function () {
					...
					if (this.layout.name === "pre-paginated" && this.layout.divisor > 1) {
						right = next.next();
						if (right && display_nextPage(next)) { //amend by jane
							return this.append(right);
						} else {
                                                    display_singlePage(next);          //amend by jane
                                                }
					}
				}.bind(this), function (err) {
					displaying.reject(err);
		...
		value: function prev() {
			...
			if (this.isPaginated && this.settings.axis === "horizontal" && (!dir || dir === "ltr")) {
				...
				this.scrollLeft = this.container.scrollLeft;
                                ...
                                // <-- amend by jane 
                                    let broswer = getBroswer();
                                    left = (broswer.broswer === 'Safari') ? (this.container.scrollLeft + this.container.scrollWidth + 1): // amend by jane (+1以免相同無法執行)
                                        (broswer.broswer === 'Firefox') ? this.container.scrollLeft :
                                        this.container.scrollLeft/* + this.container.offsetWidth*/ + Math.floor(this.layout.delta) + 1;// amend by jane (-1以免相同無法執行)
                                    // amend by jane -->
                                    if(broswer.broswer === 'Chrome')
                                        left *= (window.reader.ReaderController.displayMode() === 'Double' && $(".epub-view").length === 1 && readType === 'pre-paginated') ? 2 : 1;
                                    if(left < 0 && broswer.broswer === 'Firefox'){
                                            this.scrollBy(-this.layout.delta, 0, true); 
                                    // <-- amend by jane
                                    } else if (left </*=*/ this.container.scrollWidth && broswer.broswer !== 'Firefox') {
                                            this.scrollBy(-this.layout.delta, 0, true);
                                    } else {
                                            prev = this.views.first().section.prev();
                                    }
			...
			if (prev) {
				...
				return this.prepend(prev).then(function () {
					...
					if (this.layout.name === "pre-paginated" && this.layout.divisor > 1) {
						left = prev.prev();
						if (left && display_nextPage(prev, true)) { //amend by jane
							return this.prepend(left);
						} else {
                                                    display_singlePage(prev);               //amend by jane
                                                }
					}
				}.bind(this), function (err) {
					displaying.reject(err);
					...
					if (this.isPaginated && this.settings.axis === "horizontal") {
						if (this.settings.direction === "rtl") { 
                                                    // amend by jane -->
                                                    if(!this.settings.isVerticalWriting && (broswer.broswer === "Safari" || broswer.broswer === "Firefox")) {
		...
		value: function adjustImages(contents) {
			...
			contents.addStylesheetRules({
				"img": {
					...
					"max-width": (this._layout.columnWidth ? this._layout.columnWidth * 0.9 + "px" : "100%") + "!important",
					"max-height": (this._layout.height ? this._layout.height * 0.6 + "px" : "100%") + "!important",
					"object-fit": "contain",
					"page-break-inside": "avoid"
				},
				"svg": {
					//RDD N
					//"max-width": (this._layout.columnWidth ? this._layout.columnWidth + "px" : "80%") + "!important",	
					"max-width": (this._layout.columnWidth ? this._layout.columnWidth * 0.9 + "px" : "80%") + "!important",	
	...
	_createClass(IframeView, [{
		...
		value: function container(axis) {
			...
			element.classList.add("epub-view");
                        if(window.reader.ReaderController && window.reader.ReaderController.displayMode() === 'Single' && readType === 'pre-paginated')
					...
					if (this.settings.forceEvenPages) {
						...
						if (this.layout.divisor > 1 && this.layout.name === "reflowable" && columns % 2 > 0) {
							...
							width += this.layout.gap + this.layout.columnWidth;
                                                ...
                                                // amend by jane 修正高度不夠問題
                                                let contnetHeight = this.contents.content.clientHeight;
						height = Math.ceil(this.contents.textHeight() / contnetHeight) * contnetHeight;
...
//console.log(width, height)
		...
		value: function reframe(width, height) {
			var size;
                        /*
                        if(readType === 'pre-paginated') {
                            let newSize = reset_epubView($(this.document.head), width, height);
                            width = newSize.width;
		...
		value: function onLoad(event, promise) {
			...
			//RDD
                        this.settings.isVerticalWriting = window.reader.book.settings.isVerticalWriting; //amend by jane
			this.contents = new _contents2.default(this.document, this.document.body, this.section.cfiBase, this.section.index, this.settings.isVerticalWriting);
...
var Section = function () {
  ...
  */
   ...
   */
		...
		key: "isNodeInBody",
		value: function isNodeInBody(node) {
			var nowNode = node;
			var ancestor_first = null;
			var ancestor_second = null;
			while(nowNode.parentElement != null){
				ancestor_first = nowNode.parentElement;
				ancestor_second = nowNode;
				nowNode = nowNode.parentElement;
			}
			if(ancestor_second.nodeName == "head" || ancestor_second.nodeName == "HEAD"){
				return false;
			}else{
				return true;
			}
		...
		value: function find(_query) {
			...
			var find = function find(node) {
				...
				//** by RDD
				//check the node is searched in document body
				if(!section.isNodeInBody(node)){
					return;
				}

				// console.log(window.getComputedStyle(node).display);
				//check the node is in canvas or audio or video
				if(!!node.parentElement && 
                                        (node.parentElement.nodeName == "canvas" || node.parentElement.nodeName == "CANVAS" ||
					node.parentElement.nodeName == "audio" || node.parentElement.nodeName == "AUDIO" ||
					node.parentElement.nodeName == "video" || node.parentElement.nodeName == "VIDEO" )){
					return;
				}
				//** by RDD
				...
				var excerpt;
				var limit = 150;
                                //console.log(node.textContent.indexOf(query, last + 1));
  ...
  */
	...
	_createClass(Locations, [{
		...
		value: function process(section) {
			...
			return section.load(this.request).then(function (contents) {
				...
				//console.log("process this._locations", this._locations)
				section.unload();
   ...
   */
		...
		value: function percentageFromLocation(loc) {
			...
			// console.log(loc, this.total, loc / this.total);
   ...
   */
		...
		value: function load(locations) {
			...
			this.total = this._locations.length - 1;
			//console.log("this._locations", this._locations);
			return this._locations;
		}
   ...
   */
		...
		value: function parseMetadata(xml) {
			...
			metadata.description = this.getElementText(xml, "description");
                        metadata.generator = (xml.querySelector("[name=generator]")) ? xml.querySelector("[name=generator]").getAttribute("content") : ""; //amend by jane
			...
			metadata.viewport = this.getPropertyText(xml, "rendition:viewport");
                        //metadata.spread = this.getPropertyText(xml, "rendition:spread");// amend by jane
                        metadata.xmlSpread = this.getPropertyText(xml, "rendition:spread"); // amend by jane
...
var Navigation = function () {
  ...
  */
		...
		value: function parse(xml) {
			...
			if (!isXml) {
				...
				this.toc = this.parseNav(xml);
                                this.pagelists = this.parsePageLists(xml);
   ...
   * @private
   * @param  {document} navHtml
   * @return {array} pagelists list
   */
        }, {
                key: "parsePageLists",
		value: function parsePageLists(navHtml) {
			...
			for (i = 0; i < length; ++i) {
				item = this.pageListItem(navItems[i]);
				if (item) {
					list.push(item);
				}
			}
   ...
   */
		...
		value: function get(path) {
			...
			//var indexInUrls = this.urls.indexOf(path);
                        // Jane 添加判斷(!path) ? -1 
			 var indexInUrls = (!path) ? -1 : this.urls.indexOf(path.replace('/OEBPS/', ""));
			if (indexInUrls === -1) {
				return;
			}
		...
		value: function format(contents) {
			...
			if (this.name === "pre-paginated") {
				...
				formating = contents.columns(this.width, this.height, this.columnWidth, this.gap);
                                // amend by jane -->　流式封面設定
                                if(content_cover && contents.sectionIndex === 0)
                                    reflowable_cover(contents, this);
                                // <-- amend by jane
			} else {
				// scrolled
   ...
   */
		...
		value: function count(totalLength, pageLength) {
			...
			if (this.name === "pre-paginated") {
				...
				spreads = Math.ceil(totalLength / (pageLength / 0.8));
	...
	_createClass(Stage, [{
		...
		value: function create(options) {
			...
			container.style.position = "relative";
                        container.style.opacity = 0; // amend by jane
			...
			//**
			if (scale) {
				container.style["transform-origin"] = "center center";
		...
		value: function scale(s) {
			if (this.container) {
				this.container.style["transform-origin"] = "center center";
