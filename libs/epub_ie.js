...
function borders(el) {
	...
	widthProps.forEach(function (prop) {
		width += (style && style[prop]) ? parseFloat(style[prop]) : 0;
	});

	heightProps.forEach(function (prop) {
		height += (style && style[prop]) ? parseFloat(style[prop]) : 0;
	});
...
var Contents = function () {
	function Contents(doc, content, cfiBase, sectionIndex, isVerticalWriting/*RDD*/) {
		...
		this.epubReadingSystem("epub_ie.js", _constants.EPUBJS_VERSION);
		this.listeners();
	}
   ...
   */
		...
		value: function columns(width, height, columnWidth, gap) {
			...
			var axis = writingMode.indexOf("tb-"/*"vertical"*/) === 0 ? "vertical" : "horizontal"; //amend by jane
   ...
   */
		...
		value: function writingMode(/*mode*/) {
			...
			if (mode && this.documentElement) {
				...
				this.documentElement.style[WRITING_MODE] = (originDir === "rtl") ? "tb-rl" : "tb-lr";
                                this.documentElement.style["writing-mode"] = (originDir === "rtl") ? "tb-rl" : "tb-lr";
                                //this.documentElement.style["-epub-writing-mode"] = 'vertical-rl';
			} else {
                                this.documentElement.style[WRITING_MODE] = (originDir === "rtl") ? "rl-tb" : "lr-tb";
                                this.documentElement.style["writing-mode"] = (originDir === "rtl") ? "rl-tb" : "lr-tb";
//                                this.documentElement.style["-epub-writing-mode"] = 'horizontal-tb';
                        }
...
var DefaultViewManager = function () {
   ...
   */
				...
				if (this.layout.name === "pre-paginated" && this.layout.divisor > 1) {
					...
					if (next && display_nextPage(section)) {    //amend by jane
						return this.add(next);
                                                ...
                                                display_singlePage(section);        //amend by jane
                                        }
				}
			...
			if (this.isPaginated && this.settings.axis === "horizontal" && (!dir || dir === "ltr")) {
				...
				this.scrollLeft = this.container.scrollLeft;
                                ...
                                // <-- amend by jane 
                                    ...
                                    if (left < this.container.scrollWidth - this.layout.delta - 1 && this.container.scrollWidth / divider !== this.layout.delta/*left > 0*/) { //amend by jane
                                            this.scrollBy(-this.layout.delta /*this.layout.delta*/, 0, true); // amend by jane
                                    } else {
                                            next = this.views.last().section.next();
                                    }
                                }
				...
				this.scrollTop = this.container.scrollTop;
                                ...
                                // <-- amend by jane 
                                    ...
                                    var top = this.container.scrollTop + this.container.offsetHeight + padding.bottom;
		...
		value: function prev() {
			...
			if (this.isPaginated && this.settings.axis === "horizontal" && (!dir || dir === "ltr")) {
				...
				this.scrollLeft = this.container.scrollLeft;
                                ...
                                // <-- amend by jane 
                                    left = this.container.scrollLeft/* + this.container.offsetWidth + this.layout.delta*/; // amend by jane

                                    if (left > 0/*<= this.container.scrollWidth*/) { // amend by jane
                                            this.scrollBy(/*-*/this.layout.delta, 0, true);
                                    } else {
                                            prev = this.views.first().section.prev();
                                    }
				...
				return this.prepend(prev).then(function () {
					...
					if (this.isPaginated && this.settings.axis === "horizontal") {
						if (this.settings.direction === "rtl") {
                                                    if(!this.settings.isVerticalWriting) 
                                                        this.scrollTo(this.container.scrollWidth, 0, true, 'end');
                                                     else 
		...
		key: "scrollTo",
		value: function scrollTo(x, y, silent, end) {
			if (silent) {
				this.ignore = true;
			}
	...
	_createClass(IframeView, [{
		...
		value: function render(request, show) {
			...
			return this.sectionRender.then(function (contents) {
				...
				var axis = writingMode.indexOf("tb-"/*vertical*/) === 0 ? "vertical" : "horizontal";
					...
					if (this.settings.forceEvenPages) {
						...
						if (this.layout.divisor > 1 && this.layout.name === "reflowable" && columns % 2 > 0) {
							...
							width += this.layout.pageWidth;
                                                ...
                                                let contnetHeight = this.contents.content.offsetHeight;
  ...
  */
   ...
   */
		...
		value: function isNodeInBody(node) {
			...
			while(nowNode.parentNode != null && nowNode.nodeName.toLowerCase() !== 'html'){
				ancestor_first = nowNode.parentNode;
		...
		value: function find(_query) {
			...
			var find = function find(node) {
				...
				if(node.parentNode.nodeName == "canvas" || node.parentNode.nodeName == "CANVAS" ||
					node.parentNode.nodeName == "audio" || node.parentNode.nodeName == "AUDIO" ||
					node.parentNode.nodeName == "video" || node.parentNode.nodeName == "VIDEO" ){
					return;
				}
